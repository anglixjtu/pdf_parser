...
class PageParser(object):
        ...
        for key in ['left', 'right']:
            ...
            for line in text_lines[key]:
                page_sequence, itable = \
                    self.insert_table(
                        line.y0, tables[key], itable, page_sequence)

                page_sequence += line.get_text().replace('\n', '')
                # TODO: add period for each line 
    ...
    def get_elements(self):
            ...
            if isinstance(block, LTTextBoxHorizontal):
                ...
                for line in block._objs:
                    ...
                    if (bbox[0] >= middle) and itext > 0:
                        text_lines['right'].append(line)
                    else:
                        text_lines['left'].append(line)
        ...
        # sort
        for key in ['left', 'right']:
            text_lines[key] = self.sort_objs(text_lines[key])
            drawings[key] = self.sort_objs(drawings[key])
        ...
        for key in ['left', 'right']:
            ...
            for (i, drawing) in enumerate(self.drawings[key]):
                ...
                for iline in range(search_start, lines_num, 1):
                    text_line = self.text_lines[key][iline]
                    # TODO: SET Hyperparameter
    ...
    def merge_lines(self):
        ...
        for key in ['left', 'right']:
            ...
            while(1):
                ...
                curt_line = self.text_lines[key][i]

                if i + 1 >= lines_num:  # no need for the last
                    merged_lines[key].append(curt_line)
                    break
                next_line = self.text_lines[key][i+1]
                ...
                merged_lines[key].append(merged_line)
                i += 1
